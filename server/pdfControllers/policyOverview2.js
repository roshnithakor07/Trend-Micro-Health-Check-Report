...
const getPo2 = async (req, res) => {
  ...
  //SSPS
  switch (Report[0].configure_schedule_scan_checkbox3Policy2) {
    case 'daily':
      SPTable = `Daily - Start time: ${Report[0].configure_schedule_scan_time3} (hh:mm)`;
      break;
    case 'weeklyEvery':
      SPTable = `Weekly Every ${Report[0].configure_schedule_scan_weekevery3} - Start time: ${Report[0].configure_schedule_scan_time3} (hh:mm)`;
      break;
    case 'monthlyDay':
      SPTable = `Monthly, On day ${Report[0].configure_schedule_scan_month3} - Start time: ${Report[0].configure_schedule_scan_time3} (hh:mm)`;
      break;
    case 'monthlyOnDay':
  ...
  switch (Report[0].use_activeAction3Policy2) {
    ...
    case 'yes2':
      SSTable = [
        {
          text: 'Use the same action for all Virus/Malware type.\n(If the first action is unsuccessful, Apex One performs the second action.)',
          lineHeight: 1.2,
          margin: [13, 60, 0, 0],
          fontSize: 11,
        },
  ...
  try {
    ...
    let content = [
      ...
      {
        pageBreak: "before",
        text: `5.2.2 Policy Overview: 02 ${Report[0].OverviewPolicyName1} Policy`,
        tocItem: ['subToc'],
        style: ['heading'],
      ...
      // 1 po1 to ManualSA Half
      {
        ...
        table: {
          ...
          body: [
            ...
            ['', 'Select - Scan compressed files. > Maximum layers: 3', `Select- Scan compressed files. > Maximum layers: ${Report[0].scan_compressed_files1}`, { image: `${Report[0].tab11}`, style: "imgStyle", width: 15 }],
      ...
      // 1 ManualSA Half to 2RTSPS
      {
        ...
        table: {
          body: [
            ...
            ], RTable1, { image: `${Report[0].tab33}`, style: "imgStyle", width: 15, margin: [0, 60, 0, 0] }],
...

      {
        ...
        table: {
          body: [
            // bm
            [{ text: 'Behavior Monitoring', rowSpan: 7, margin: [0, 140, 0, 0] }, 'Enable Malware Behavior Blocking Known and potential threats', `Enabled`, { image: `${tab1}`, style: "imgStyle", width: 15 }],
            ['', 'Ransomware Protection Protect documents against unauthorized encryption or modification', `Enabled`, { image: `${Report[0].tab51}`, style: "imgStyle", width: 15 }],
            ['', 'Protect documents against unauthorized encryption or modification -> Automatically backup and restore files changed by suspicious programs', `Enabled`, { image: `${tab1}`, style: "imgStyle", width: 15 }],
            ['', 'Enable program inspection to detect and block compromised executable files', `Enabled`, { image: `${tab1}`, style: "imgStyle", width: 15 }],
            ['', 'Anti-exploit Protection Terminate programs that. exhibit abnormal behavior. associated with exploit attacks', `Enabled`, { image: `${tab1}`, style: "imgStyle", width: 15 }],
            ['', 'Newly Encountered Programs Monitor newly encountered programs downloaded through web or email application channels -> Prompt User', `Enabled`, { image: `${tab1}`, style: "imgStyle", width: 15 }],
            ['', 'Exceptions (specify exception if any)', `Enabled`, { image: `${tab1}`, style: "imgStyle", width: 15 }],
            ...
            ['', 'Action File: quarantine Process: Terminate', `Action Set File: quarantine Process: Terminate`, { image: `${tab1}`, style: "imgStyle", width: 15 }],
            ['', 'Exception (specify exception if any)', `No Exceptions added`, { image: `${tab1}`, style: "imgStyle", width: 15 }],
            ...
            ['Vulnerability Protection', 'Enable', `${Report[0].vulnerability_protection} with TAP Mode`, { image: `${Report[0].tabVP}`, style: "imgStyle", width: 15 }],
...

      {
        ...
        table: {
          body: [
            ...
            { image: `${Report[0].tab59}`, style: "imgStyle", width: 15, margin: [0, 100, 0, 0] }],
          ]
        }
      },
