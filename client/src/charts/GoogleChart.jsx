...
export default function GoogleChart(props) {
  const { getCcApi, bmApi, getReportData } = Endpoints();
  ...
  const heading = ["Risk Level", "Subject","Event Type","Target","Action","Operation","EndPoint"]
  ...
  if (columnsNames === vm[0] || columnsNames === vm[1]) {
    ...
    for (let i = 0; i < totalActions.length; i++) {
      ...
      switch (totalActions[i]) {
        case "Assess":
          totalActions[i] = "Assessed";
          break;

        case "Deny":
          totalActions[i] = "Denied";
          break;

        case "Clean ":
          totalActions[i] = "Cleaned";
          break;

        case "Terminate":
          totalActions[i] = "Terminated";
          break;
        default:
          break;
      }
    ...
    if (columnsNames === vm[0]) {
      ...
      description[1][0] = `${y1[0]} times ${x[0]} is triggered ${endPointVal0}, and it is ${actionVal0} by Apex one.`;
      description[1][1] = `${y1[1]} times ${x[1]} is triggered ${endPointVal1}, and it is ${actionVal1} by Apex one.`;
    }
